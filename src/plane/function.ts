...
export function func(fn: (x: number) => number, xRange: [number, number]) {
  ...
  function generatePath() {
    ...
    for (let x = start; x <= end; x += step) {
      try {
        const y = fn(x);
        if (isFinite(y) && (yRange === null || (y >= yRange[0] && y <= yRange[1]))) {
          points.push([
            x * scaleX + offsetX,
            -y * scaleY + offsetY
          ]);
  ...
  function derivative() {
    ...
    const derivPath = func(derivFn, xRange)
      .style({
  ...
  function integral(from: number = xRange[0]) {
    ...
    const theme = getTheme();
    const integralPath = func(integralFn, xRange)
      .style({
  ...
  function highlight(from: number, to: number, color: string = 'rgba(255,0,0,0.2)') {
